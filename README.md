...
**Зачем отключена проверка — ради архитектуры stealth:**

1. **Экономия:** не нужен свой домен, DNS, Certbot и продление Let's Encrypt — достаточно VPS с IP.
2. **Маскировка под чужой SNI:** сервер мимикрирует под крупный легитимный ресурс (`go.dev`, `docker.com` и т.п.) с `host-resolver-rules`; для зонда это выглядит как обращение к известному сервису, а не к «вашему.домену.com».
3. **Reality-like защита от зондирования** — главная причина: неавторизованные подключения Naive Server **прозрачно проксирует** на реальный SNI (TCP и QUIC), и сканер видит валидный ответ настоящего сайта. Официальный `klzgrad/naiveproxy` + Caddy forwardproxy так не умеют: там нужен **ваш** домен с **вашим** сертификатом, а посторонний зонд получает decoy или отказ — прокси проще вычислить.
...
Это осознанная плата за stealth и Reality-like маскировку (см. выше), но убирает криптографическую гарантию «на другом конце именно мой сервер, а не чужой».
